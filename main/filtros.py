...
class ProcessoFilter(filters.FilterSet):	
	...
	componente = filters.CharFilter(field_name='macroProcesso_primario__componente_primario__tipo', lookup_expr='contains')
	
	class Meta:
		model = Processo
		fields = ['cod_processo','processo_nome','macroprocesso', 'proprietario', 'gestorPrincipal', "componente"]
...
class ComponenteFilter(filters.FilterSet):
	...
	componente = filters.CharFilter(field_name='tipo', lookup_expr='contains')

	class Meta: 
		model = Componente
		fields = ['nome_componente', 'componente', 'cod_componente']
